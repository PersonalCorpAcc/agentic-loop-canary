---
# Managed by @plainconceptsplatform/workflows@0.5.1. Source: loops/workflows/shared/opencode-ci.md. Profile digest: 477494a3b05c. Update with `workflows update --force`; consumer edits may be overwritten.
env:
  RTK_VERSION: "0.44.1"
...
pre-agent-steps:
  ...
  - name: Runner preflight
    # Every baseline step is user-space, so a runner that lacks one of these fails
    # later with a message about something else: a sandbox that will not start, an
    # install into a read-only prefix, a classifier that reads every label as absent.
    # Say which one is missing, once, here (FR-070). The sandbox also names its
    # containers, and a persistent runner keeps them between jobs.
    run: |
      set -euo pipefail
      missing=""
      docker info >/dev/null 2>&1 || missing="the container daemon"
      [ -n "$missing" ] || docker compose version >/dev/null 2>&1 || missing="the Docker Compose plugin"
      [ -n "$missing" ] || command -v jq >/dev/null 2>&1 || missing="jq"
      [ -n "$missing" ] || { mkdir -p "$HOME/.local" && [ -w "$HOME/.local" ]; } || missing="a writable \$HOME/.local"
      if [ -n "$missing" ]; then
        echo "::error::Runner preflight failed: this runner has no $missing, and every baseline step is user-space and needs it."
        exit 1
      fi
      stale="$(docker ps -aq --filter 'name=^awf-' --filter 'name=^awmg-' 2>/dev/null || true)"
      if [ -n "$stale" ]; then
        # shellcheck disable=SC2086
        docker rm -f $stale >/dev/null
        echo "Removed $(printf '%s\n' "$stale" | wc -l | tr -d ' ') sandbox container(s) a previous job left behind."
      fi
